...
export async function sendFriendRequest(req, res) {
    try {
        ...
        const result = await sql`INSERT INTO friend_requests (sender_id,reciever_id,created_at) VALUES (${senderId},${recieverId},NOW())`;
...
export async function getFriendRequests(req,res) {
    try {
        const {userId} = req.body;
        const friendRequests = await sql`SELECT * FROM friend_requests WHERE recieverId = ${userId}`;
        return res.status(200).json({
            status: 'success',
...
export async function getSuggestions(req,res) {
    try {
        const suggestions = await sql`SELECT userID,username FROM users ORDER BY RAND() LIMIT 10 where userID != ${req.body.userId}`;
        return res.status(200).json({
            status: 'success',
